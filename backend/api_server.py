...
def query():
    ...
    if query_type == 'grammar':
        ...
        if query_type == 'c':
            ...
            messages = [
                {
                    "role": "system",
                    "content": "You are an AI writing assistant. Continue the following text by adding 3-5 words in a coherent and natural way, expanding the ideas logically. Do not provide explanations. Don't repeat suggestions."
                               f"{instructions}\n\n"
                               f"Prompt:\n{prompt_text}\n\n"
                },
                {
    ...
    try:
        response = openai.ChatCompletion.create(
            model=engine, #model=model_name didn't work
            messages=messages,
